...
export function AuthFlow({ onSuccess }: AuthFlowProps) {
  ...
  return (
    <div className="flex-1 flex flex-col md:flex-row min-h-screen bg-amwal-neutral-light font-jakarta">
      
      {/* ================= PANEL KIRI (Desktop Only - Branding banner) ================= */}
      <div className="hidden md:flex md:w-1/2 bg-gradient-to-br from-amwal-secondary-teal to-[#1b3537] flex-col items-center justify-center p-12 text-white relative overflow-hidden">
        
        {/* Glow ambient background lights */}
        <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-amwal-secondary-green/10 rounded-full filter blur-3xl pointer-events-none" />
        <div className="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-amwal-secondary-green/10 rounded-full filter blur-3xl pointer-events-none animate-pulse" />
        
        <div className="max-w-md text-center flex flex-col items-center z-10">
          {/* Glowing Brand Emblem */}
          <div className="relative w-36 h-36 mb-8 rounded-amwal-lg bg-white/5 border border-white/10 p-3 shadow-[0_8px_32px_rgba(0,0,0,0.15)] backdrop-blur-md flex items-center justify-center">
            <Image
          ...
          <h1 className="text-3xl font-yeseva tracking-tight leading-tight mb-4">
            Keuangan Syariah Lebih Berkah
          </h1>
          <p className="text-base text-amwal-main-1/80 font-medium leading-relaxed max-w-sm">
        ...
        <div className="absolute bottom-8 left-12 text-xs text-amwal-main-1/50 font-semibold tracking-wide">
      ...
      <div className="flex-1 flex items-center justify-center p-0 sm:p-6 md:w-1/2">
        <div className="w-full h-screen sm:h-auto sm:max-w-[430px] bg-white sm:rounded-amwal-lg px-6 sm:px-8 py-8 flex flex-col justify-between md:justify-center md:gap-8 shadow-[0_15px_40px_rgba(0,0,0,0.02)] border-0 sm:border border-amwal-main-2/20 overflow-y-auto">
            ...
            {mode === "register" ? (
              ...
              <form onSubmit={handleRegister} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-bold text-amwal-secondary-teal">Email atau telepon</label>
                  <input
                    type="text"
                    value={regEmail}
                    onChange={(e) => setRegEmail(e.target.value)}
                    placeholder="Masukkan email atau nomor yang benar"
                    className={`w-full h-[50px] px-4 rounded-amwal-sm border bg-transparent text-sm font-medium outline-none transition-all ${
                      regErrors.email
                        ? "border-amwal-status-danger bg-amwal-status-danger/5 focus:border-amwal-status-danger focus:ring-1 focus:ring-amwal-status-danger"
                        : "border-slate-200 focus:border-amwal-secondary-teal focus:ring-1 focus:ring-amwal-secondary-teal/30"
                    }`}
                  />
                  {regErrors.email && (
                ...
                {/* Nama */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-bold text-amwal-secondary-teal">Nama</label>
                  <input
                    type="text"
                    value={regNama}
                ...
                {/* Sandi */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-bold text-amwal-secondary-teal">Sandi</label>
                  <div className="relative">
                    <input
                      ...
                      className={`w-full h-[50px] pl-4 pr-11 rounded-amwal-sm border bg-transparent text-sm font-medium outline-none transition-all ${
                        regErrors.sandi
                          ? "border-amwal-status-danger bg-amwal-status-danger/5 focus:border-amwal-status-danger focus:ring-1 focus:ring-amwal-status-danger"
                          : "border-slate-200 focus:border-amwal-secondary-teal focus:ring-1 focus:ring-amwal-secondary-teal/30"
                      }`}
                    ...
                    <button
                      ...
                      className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-amwal-secondary-teal transition-colors focus:outline-none cursor-pointer"
                    >
                      {showRegSandi ? (
                        /* Eye Open */
                ...
                {/* Ulangi Sandi */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-bold text-amwal-secondary-teal">Ulangi sandi</label>
                  <div className="relative">
                    <input
            ...
            {mode === "register" ? (
              <>
                ...
                <button
                  ...
                  className="text-amwal-secondary-teal font-bold ml-1 hover:underline focus:outline-none cursor-pointer"
                >
                  Masuk
                </button>
              </>
