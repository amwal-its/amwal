...
## Putaran 11 — Wiring Full-Stack Program Wakaf & Audit Escrow (Sprint 9 Task 9.4)

| # | Topik | Keputusan & Catatan Teknis |
|---|---|---|
| 1 | Full-Stack CRUD Program Wakaf via Prisma | Seluruh mock local state pada `WakafProgramsView` dan `app/(backoffice)/admin/wakaf/page.tsx` telah digantikan dengan integrasi 100% database PostgreSQL Prisma. Pembuatan program baru mengeksekusi inisialisasi atomik `WaqfProgram` + `WaqfPrincipalLedger` + `ProgramProgressReport`. |
| 2 | Slider Progres Fisik Real-time & Debounce API | Interaksi slider progres fisik proyek (0% - 100%) terhubung ke endpoint `POST /api/admin/wakaf/programs/[id]/progress`, mencatat persentase fisik dan riwayat audit di tabel `ProgramProgressReport`. |
| 3 | Kuitansi Belanja Digital & Audit DPS | Sub-tab kuitansi belanja terhubung ke `POST /api/admin/wakaf/programs/[id]/receipts` dan `PATCH .../receipts`, menyimpan array nota belanja terverifikasi pada JSON `kuitansiUrls`. |
| 4 | Tata Kelola Termin Escrow & Surplus Wakaf Produktif | Persetujuan dan penolakan pencairan termin kas escrow terhubung penuh ke `PATCH /api/admin/withdrawal-requests/[id]` dengan penegakan fiqih wakaf (HABIS_PAKAI mengurangi pokok, PRODUKTIF_KEKAL mengurangi available yield dan menjaga pokok tetap utuh). Ditambahkan modal pencatatan `WaqfYieldEntry` untuk surplus wakaf produktif. |
| 5 | Guarding & Data Integrity | Semua endpoint mutasi program wakaf dan termin escrow dilindungi validasi sesi role `ADMIN`. |

## Putaran 12 — Payment Production Hardening & WhatsApp Daemon Engine (Sprint 9 Task 9.1 & 9.2)

| # | Topik | Keputusan & Catatan Teknis |
|---|---|---|
| 1 | Hardening Midtrans Webhook & Anti-Tampering SHA-512 | Endpoint `POST /api/webhooks/payment` di-hardened dengan verifikasi signature SHA-512 resmi Midtrans (`SHA512(order_id + status_code + gross_amount + MIDTRANS_SERVER_KEY)`). Request dengan signature tidak valid otomatis ditolak `403 Forbidden` tanpa mutasi data. |
| 2 | Transaksi Atomik Prisma Berjenjang | Status Midtrans dipetakan ke `TransactionPaymentStatus`: `settlement`/`capture` $\rightarrow$ `LUNAS`, `pending` $\rightarrow$ `PENDING`, `expire`/`cancel`/`deny` $\rightarrow$ `GAGAL`. Mutasi `Transaction`, `WaqfOrder` + `WaqfPrincipalLedger`, `ZakatOrder` + `FundPool`, dan `QurbanOrder` + `QurbanAnimalSlot` dikemas dalam `prisma.$transaction`. |
| 3 | Arsitektur Baileys WhatsApp Microservice Terpisah | Baileys engine dioperasikan sebagai service mandiri (`amwal-baileys-service`, port 4001) untuk menjamin koneksi WebSocket persistent dan isolasi filesystem session (`auth_session/`). Komunikasi dari `amwalheti` menggunakan HTTP internal dengan proteksi `X-Internal-Secret`. |
| 4 | Panel Live QR Scanner & Admin Tester | Di `/admin/pengaturan` (Tab Gateway Notifikasi & WA), dipasang panel interaktif live polling status koneksi (`WAITING_FOR_SCAN`, `CONNECTED`, `OFFLINE`), container visual QR Code render langsung, dan live form tester pengiriman WhatsApp. |
| 5 | Non-blocking WhatsApp Best-Effort Dispatch | Trigger notifikasi WhatsApp via `lib/notification-templates.ts` dan `lib/whatsapp.service.ts` diisolasi di luar DB transaction. Kegagalan atau timeout koneksi WA (timeout 5s) tidak menggagalkan status pelunasan transaksi di DB dan tetap merespon `200 OK` ke Midtrans. |
